...
# 조회버튼 클릭 ##monthPeriodDiv > span:nth-child(2) > input[type=button] 
# 속성 중에 disalble = True 인 엘리먼트는 사용자 클릭을 막아놓음
btn = driver.find_element(By.XPATH,'//*[@id="monthPeriodDiv"]/span[2]/input')
driver.execute_script('argument[0].click()', btn) # 사용자 마우스 클릭 이벤트를 발생
# driver.execute_script('selectRedord('period')') 직접 실행

time.sleep(3)
more = driver.find_element(By.XPATH,'//*[@id="autodanawa_gridC"]/div[3]/article/main/div/div[2]/div[1]/a')
more.click(0)

driver.page_source  # --> 셀리니움 문법을 이용해서 원하는 태그의 속한 텍스트를 추출
